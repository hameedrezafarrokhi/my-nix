...
equalize() {
  ...
  local third_size=$(jget height "$(bspc query -T -n $(bspc query -N '@/2' -n .descendant_of.window.$node_filter | head -n 1))")
  local total_size=$((  first_size + second_size + third_size  ))
  local want=$(( total_size / 3 ))
  bspc node $(bspc query -N '@/2' -n .descendant_of.window.$node_filter | head -n 1) -z top 0 $((third_size - want));
