...
equalize() {
  ...
  local fifth_size=$(jget width "$(bspc query -T -n $(bspc query -N '@/2/2/1' -n .descendant_of.window.$node_filter | head -n 1))")
  local sixth_size=$(jget width "$(bspc query -T -n $(bspc query -N '@/2/2/2' -n .descendant_of.window.$node_filter | head -n 1))")
  ...
  bspc node $(bspc query -N '@/1/2/2' -n .descendant_of.window.$node_filter| head -n 1) -z right $((want - third_size)) 0
  bspc node $(bspc query -N '@/2/1' -n .descendant_of.window.$node_filter| head -n 1) -z right $((want - fourth_size)) 0

 #bspc node $(bspc query -N '@/2/2/2' -n .descendant_of.window.$node_filter | head -n 1) -z left $((sixth_size - want)) 0
  bspc node $(bspc query -N '@/2/2/1' -n .descendant_of.window.$node_filter | head -n 1) -z right $((want - fifth_size)) 0
 #
 #
...
window_count=$(bspc query -N -n .window.$node_filter -d focused | wc -l)
  if (( window_count < 6 )); then
    ...
    rotate '@/2/2/1' horizontal 90
    rotate '@/2/2/2' horizontal 90
    calculate
    equalize
    equalize
    equalize
