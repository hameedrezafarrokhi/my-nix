...
equalize() {
  ...
  local fifth_size=$(jget height "$(bspc query -T -n $(bspc query -N '@/2/2' -n .descendant_of.window.$node_filter | head -n 1))")
  local total_size=$((  first_size + second_size + third_size + fourth_size + fifth_size ))
  local want=$(( total_size / 5 ))
  bspc node $(bspc query -N '@/2/2' -n .descendant_of.window.$node_filter | head -n 1) -z top 0 $((fifth_size - want));
  bspc node $(bspc query -N '@/2/1' -n .descendant_of.window.$node_filter | head -n 1) -z top 0 $((fourth_size - want));
