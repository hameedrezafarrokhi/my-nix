...
equalize() {
  ...
  local have_left=$(jget width "$(bspc query -T -n $(bspc query -N '@/1/2' -n .descendant_of.window.$node_filter | head -n 1))")

  if (( window_count > 2 )); then
      bspc node $(bspc query -N '@/1/1' -n .descendant_of.window.$node_filter | head -n 1) -z right $((want_master - have)) 0;
      bspc node $(bspc query -N '@/1/2' -n .descendant_of.window.$node_filter| head -n 1) -z right $((want - have_left)) 0;
  else
     auto_balance '@/'
  fi
}
