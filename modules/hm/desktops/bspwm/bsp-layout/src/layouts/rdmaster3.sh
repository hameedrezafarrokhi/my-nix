...
equalize() {
  ...
  rotate '@/2' horizontal 90
  local left_stack_node=$(bspc query -N '@/2' -n)
  for parent in $(bspc query -N '@/2' -n .descendant_of.$node_filter | grep -v $left_stack_node); do
  rotate $parent horizontal 90
  done
...
calculate() {
  ...
  if (( $(bspc query -N '@/2' -n .descendant_of.window.$node_filter | wc -l) > 3 )); then
