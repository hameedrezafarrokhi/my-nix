...
equalize() {
  ...
  local second_size=$(jget width "$(bspc query -T -n $(bspc query -N '@/1/2/1' -n .descendant_of.window.$node_filter | head -n 1))")
  local third_size=$(jget width "$(bspc query -T -n $(bspc query -N '@/1/2/2' -n .descendant_of.window.$node_filter | head -n 1))")
  local fourth_size=$(jget width "$(bspc query -T -n $(bspc query -N '@/2/1' -n .descendant_of.window.$node_filter | head -n 1))")
  local fifth_size=$(jget width "$(bspc query -T -n $(bspc query -N '@/2/2' -n .descendant_of.window.$node_filter | head -n 1))")
  local total_size=$((  first_size + second_size + third_size + fourth_size + fifth_size ))
  local want=$(( total_size / 5 ))
  bspc node $(bspc query -N '@/2/2' -n .descendant_of.window.$node_filter | head -n 1) -z left $((fifth_size - want)) 0;
  bspc node $(bspc query -N '@/2/1' -n .descendant_of.window.$node_filter | head -n 1) -z left $((fourth_size - want)) 0;
  bspc node $(bspc query -N '@/1/1' -n .descendant_of.window.$node_filter| head -n 1) -z right $((want - first_size)) 0;
  bspc node $(bspc query -N '@/1/2/1' -n .descendant_of.window.$node_filter| head -n 1) -z right $((want - second_size)) 0;

 #bspc node $(bspc query -N '@/2/1' -n .descendant_of.window.$node_filter| head -n 1) -z right $((fourth_size - want)) 0;
}

calculate() {
# bootstrap: ensure required splits exist
# need:
# - @/1 has at least 3 windows (so @/1/1 and @/1/2 exist)
# - @/1/2 has at least 2 windows (so @/1/2/1 and @/1/2/2 exist)
# - @/2 has at least 2 windows (so @/2/1 and @/2/2 exist)
...
# extras priority: S1 → S2 → S3 → S4 → S5
t1=$(( base + (rem >= 1) ))
t2=$(( base + (rem >= 2) ))
t3=$(( base + (rem >= 3) ))
t4=$(( base + (rem >= 4) ))
...
window_count=$(bspc query -N -n .window.$node_filter -d focused | wc -l)
  if (( window_count < 5 )); then
    ...
    rotate '@/1/2/1' horizontal 90
    rotate '@/1/2/2' horizontal 90
