...
execute_layout() {
 ...
 #rotate '@/2' vertical 90
  ...
  local master_stack_node=$(bspc query -N '@/2' -n);
  ...
  local have=$(jget width "$(bspc query -T -n $(bspc query -N '@/2' -n .descendant_of.window.$node_filter | head -n 1))");  #WARNING CHANGED

  bspc node $(bspc query -N '@/2' -n .descendant_of.window.$node_filter | head -n 1) --resize left $((have - want)) 0;  #WARNING CHANGED
}
