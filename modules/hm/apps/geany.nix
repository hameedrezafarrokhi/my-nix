  ...
  xdg.configFile = {
    ...
    geany-conf = {
      ...
      text = lib.mkBefore ''
        [plugins]
        load_plugins=true
        custom_plugin_path=${config.xdg.configHome}/geany/plugin-packages

        [tools]
        terminal_cmd=kitty sh -c "/bin/sh %c; exec $SHELL"
        browser_cmd=${config.my.default.browser-alt-name} %c
        grep_cmd=grep
        ...
        pref_template_developer=${admin}
        pref_template_company=
        pref_template_mail=${config.programs.git.settings.user.email}
        pref_template_initial=h
        ...
        indent_type=1
        virtualspace=1
        change_history_markers=true
        change_history_indicators=true
