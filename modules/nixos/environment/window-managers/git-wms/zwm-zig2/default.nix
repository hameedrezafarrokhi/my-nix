...
let
  ...
  cfg = config.services.xserver.windowManager.zwm-zig2;
  zwm-zig2 = pkgs.callPackage ./zwm-zig2.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.zwm-zig2 = {
      enable = mkEnableOption "zwm-zig2";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before zwm-zig2 is started.
        '';
      };
      package = mkPackageOption pkgs "zwm-zig2" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "zwm-zig2" config.my.window-managers) {
