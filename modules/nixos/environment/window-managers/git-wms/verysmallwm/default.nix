...
let
  ...
  cfg = config.services.xserver.windowManager.verysmallwm;
  verysmallwm = pkgs.callPackage ./verysmallwm.nix {
    conf = ''
#!/usr/bin/env python3
...

{
  ...
  options = {
    services.xserver.windowManager.verysmallwm = {
      enable = mkEnableOption "verysmallwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before verysmallwm is started.
        '';
      };
      package = mkPackageOption pkgs "verysmallwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "verysmallwm" config.my.window-managers) {
