...
let
  ...
  cfg = config.services.xserver.windowManager.mmwm;
  akita_mmwm = pkgs.writeShellScriptBin "akita_mmwm" ''
    ${builtins.readFile ./akita_mmwm}
  '';
  mmwm = pkgs.callPackage ./mmwm.nix {
    conf = ''
#ifndef CONFIG_H
#define CONFIG_H
...

{
  ...
  options = {
    services.xserver.windowManager.mmwm = {
      enable = mkEnableOption "mmwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before mmwm is started.
        '';
      };
      package = mkPackageOption pkgs "mmwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "mmwm" config.my.window-managers) {
