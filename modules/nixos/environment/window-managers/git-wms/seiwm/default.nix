...
let
  ...
  cfg = config.services.xserver.windowManager.seiwm;
  seiwm = pkgs.callPackage ./seiwm.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.seiwm = {
      enable = mkEnableOption "seiwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before seiwm is started.
        '';
      };
      package = mkPackageOption pkgs "seiwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "seiwm" config.my.window-managers) {
