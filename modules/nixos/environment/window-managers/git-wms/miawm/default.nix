...
let
  ...
  cfg = config.services.xserver.windowManager.miawm;
  miawm = pkgs.callPackage ./miawm.nix {
    conf = ''
// ----------------------------------------------------------------------------------------------------------------------------#
...

{
  ...
  options = {
    services.xserver.windowManager.miawm = {
      enable = mkEnableOption "miawm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before miawm is started.
        '';
      };
      package = mkPackageOption pkgs "miawm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "miawm" config.my.window-managers) {
