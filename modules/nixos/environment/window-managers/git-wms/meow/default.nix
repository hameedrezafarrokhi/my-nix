...
let
  ...
  cfg = config.services.xserver.windowManager.meow;
  meow = pkgs.callPackage ./meow.nix {
    conf = ''
#pragma once
#include <X11/X.h>
#include <limits.h>
...
#define BORDER_FOCUSED_WIDTH 4 // currently a bit broken visually(will look ugly)
...
#define MOD Mod4Mask // alt by default

// ===define values HERE ===
// keepsame |  [name]   |   [args] (if space is present seperate by "arg1", "arg2")
static char *terminal[] = {"${config.my.default.terminal}", NULL};
static char *launcher[] = {"rofi","-show","drun","-theme",".config/rofi/themes/main.rasi", NULL};
...

{
  ...
  options = {
    services.xserver.windowManager.meow = {
      enable = mkEnableOption "meow";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before meow is started.
        '';
      };
      package = mkPackageOption pkgs "meow" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "meow" config.my.window-managers) {
