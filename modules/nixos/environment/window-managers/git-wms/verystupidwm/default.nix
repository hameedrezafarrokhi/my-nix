...
let
  ...
  cfg = config.services.xserver.windowManager.verystupidwm;
  verystupidwm = pkgs.callPackage ./verystupidwm.nix {
    conf = ''
#include <signal.h>
#include <stdlib.h>
#include <unistd.h>
...

{
  ...
  options = {
    services.xserver.windowManager.verystupidwm = {
      enable = mkEnableOption "verystupidwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before verystupidwm is started.
        '';
      };
      package = mkPackageOption pkgs "verystupidwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "verystupidwm" config.my.window-managers) {
