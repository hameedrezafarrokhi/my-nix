...
let
  ...
  cfg = config.services.xserver.windowManager.monsterwm-xcb;
  monsterwm-xcb = pkgs.callPackage ./monsterwm-xcb.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.monsterwm-xcb = {
      enable = mkEnableOption "monsterwm-xcb";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before monsterwm-xcb is started.
        '';
      };
      package = mkPackageOption pkgs "monsterwm-xcb" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "monsterwm-xcb" config.my.window-managers) {
