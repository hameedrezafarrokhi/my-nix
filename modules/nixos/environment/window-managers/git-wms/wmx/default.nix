...
let
  ...
  cfg = config.services.xserver.windowManager.wmx;
  wmx = pkgs.callPackage ./wmx.nix {
    conf = ''
#ifndef _CONFIG_H_
#define _CONFIG_H_
...

{
  ...
  options = {
    services.xserver.windowManager.wmx = {
      enable = mkEnableOption "wmx";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before wmx is started.
        '';
      };
      package = mkPackageOption pkgs "wmx" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "wmx" config.my.window-managers) {
