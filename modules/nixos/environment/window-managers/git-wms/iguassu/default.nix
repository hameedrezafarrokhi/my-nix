...
let
  ...
  cfg = config.services.xserver.windowManager.iguassu;
  iguassu = pkgs.callPackage ./iguassu.nix {
   conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.iguassu = {
      enable = mkEnableOption "iguassu";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before iguassu is started.
        '';
      };
      package = mkPackageOption pkgs "iguassu" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "iguassu" config.my.window-managers) {
