...
let
  ...
  cfg = config.services.xserver.windowManager.mdtwm;
  mdtwm = pkgs.callPackage ./mdtwm.nix {
...

{
  ...
  options = {
    services.xserver.windowManager.mdtwm = {
      enable = mkEnableOption "mdtwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before mdtwm is started.
        '';
      };
      package = mkPackageOption pkgs "mdtwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "mdtwm" config.my.window-managers) {
