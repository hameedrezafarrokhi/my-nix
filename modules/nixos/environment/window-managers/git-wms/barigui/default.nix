...
let
  ...
  cfg = config.services.xserver.windowManager.barigui;
  barigui = pkgs.callPackage ./barigui.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.barigui = {
      enable = mkEnableOption "barigui";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before barigui is started.
        '';
      };
      package = mkPackageOption pkgs "barigui" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "barigui" config.my.window-managers) {
