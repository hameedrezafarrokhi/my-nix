...
let
  ...
  wtftw-contrib = pkgs.callPackage ./wtftw-contrib.nix {
    wtftw = wtftw;
  };
  wtftw = pkgs.callPackage ./wtftw.nix {
    conf = ''
#[macro_use]
extern crate wtftw;
...
pub extern fn configure(_: &mut WindowManager, w: &dyn WindowSystem, config: &mut Config) {
    let modm = KeyModifiers::MOD4MASK;

    config.general.mod_mask = modm;
    config.general.border_color = 0x404040;
    config.general.focus_border_color = 0x436b7b;
...

{
  ...
  options = {
    services.xserver.windowManager.wtftw = {
      enable = mkEnableOption "wtftw";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before wtftw is started.
        '';
      };
      package = mkPackageOption pkgs "wtftw" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "wtftw" config.my.window-managers) {
