...
let
  ...
  cfg = config.services.xserver.windowManager.karuiwm;
  karuiwm = pkgs.callPackage ./karuiwm.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.karuiwm = {
      enable = mkEnableOption "karuiwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before karuiwm is started.
        '';
      };
      package = mkPackageOption pkgs "karuiwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "karuiwm" config.my.window-managers) {
