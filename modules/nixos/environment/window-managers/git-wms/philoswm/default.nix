...
let
  ...
  cfg = config.services.xserver.windowManager.philoswm;
  philoswm = pkgs.callPackage ./philoswm.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.philoswm = {
      enable = mkEnableOption "philoswm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before philoswm is started.
        '';
      };
      package = mkPackageOption pkgs "philoswm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "philoswm" config.my.window-managers) {
