...
let
  ...
  cfg = config.services.xserver.windowManager.sara;
  sara = pkgs.callPackage ./sara.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.sara = {
      enable = mkEnableOption "sara";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before sara is started.
        '';
      };
      package = mkPackageOption pkgs "sara" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "sara" config.my.window-managers) {
