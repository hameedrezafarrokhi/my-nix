...
let
  ...
  cfg = config.services.xserver.windowManager.eowm;
  eowm = pkgs.callPackage ./eowm.nix {
    conf = ''
// appearance
...

{
  ...
  options = {
    services.xserver.windowManager.eowm = {
      enable = mkEnableOption "eowm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before eowm is started.
        '';
      };
      package = mkPackageOption pkgs "eowm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "eowm" config.my.window-managers) {
