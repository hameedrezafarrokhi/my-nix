...
let
  ...
  cfg = config.services.xserver.windowManager.dminiwm;
  dminiwm = pkgs.callPackage ./dminiwm.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.dminiwm = {
      enable = mkEnableOption "dminiwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before dminiwm is started.
        '';
      };
      package = mkPackageOption pkgs "dminiwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "dminiwm" config.my.window-managers) {
