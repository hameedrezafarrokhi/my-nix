...
let
  ...
  cfg = config.services.xserver.windowManager.cluless;
  clubar = pkgs.callPackage ./clubar.nix {
    conf = ''
...
#undef BAR_HEIGHT
  ...
  cluless = pkgs.callPackage ./cluless.nix {
    conf = ''
// {{{ Includes.
#include <X11/Xutil.h>
...

{
  ...
  options = {
    services.xserver.windowManager.cluless = {
      enable = mkEnableOption "cluless";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before cluless is started.
        '';
      };
      package = mkPackageOption pkgs "cluless" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "cluless" config.my.window-managers) {
    ...
    environment.systemPackages = [ cfg.package clubar ];
