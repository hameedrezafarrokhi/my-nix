...
let
  ...
  cfg = config.services.xserver.windowManager.pwm;
  pwm = pkgs.callPackage ./pwm.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.pwm = {
      enable = mkEnableOption "pwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before pwm is started.
        '';
      };
      package = mkPackageOption pkgs "pwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "pwm" config.my.window-managers) {
