...
let
  ...
  cfg = config.services.xserver.windowManager.fleon;
  fleon = pkgs.callPackage ./fleon.nix {
    conf = ''
...

{
  ...
  options = {
    services.xserver.windowManager.fleon = {
      enable = mkEnableOption "fleon";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before fleon is started.
        '';
      };
      package = mkPackageOption pkgs "fleon" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "fleon" config.my.window-managers) {
