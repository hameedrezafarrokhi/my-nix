...
let
  ...
  cfg = config.services.xserver.windowManager.xswm;
  xswm-bin = pkgs.writeShellScriptBin "xswm" ''
    ${xswm}/xswm
  '';
  xswm = pkgs.callPackage ./xswm.nix {
    conf = ''
#include <signal.h>
#include <stdio.h>
#include <stdlib.h>
...

{
  ...
  options = {
    services.xserver.windowManager.xswm = {
      enable = mkEnableOption "xswm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before xswm is started.
        '';
      };
      package = mkPackageOption pkgs "xswm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "xswm" config.my.window-managers) {
    ...
    environment.systemPackages = [ cfg.package xswm-bin ];
