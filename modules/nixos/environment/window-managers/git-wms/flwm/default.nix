...
let
  ...
  cfg = config.services.xserver.windowManager.flwm;
  flwm = pkgs.callPackage ./flwm.nix {
    conf = ''
// config.h
...

{
  ...
  options = {
    services.xserver.windowManager.flwm = {
      enable = mkEnableOption "flwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before flwm is started.
        '';
      };
      package = mkPackageOption pkgs "flwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "flwm" config.my.window-managers) {
