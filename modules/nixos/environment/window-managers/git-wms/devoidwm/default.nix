...
let
  ...
  cfg = config.services.xserver.windowManager.devoidwm;
  devoidwm = pkgs.callPackage ./devoidwm.nix {

    patches = [
      #./devoidwm/patches/center.diff
      #./devoidwm/patches/control_gaps.diff
    ];
...

{
  ...
  options = {
    services.xserver.windowManager.devoidwm = {
      enable = mkEnableOption "devoidwm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before devoidwm is started.
        '';
      };
      package = mkPackageOption pkgs "devoidwm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "devoidwm" config.my.window-managers) {
