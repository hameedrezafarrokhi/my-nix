...
let
  ...
  cfg = config.services.xserver.windowManager.howm;

  hobar = pkgs.writeShellScriptBin "hobar" ''
    ${builtins.readFile ./bar.sh}
  '';

  howm = pkgs.callPackage ./howm.nix {

    cottage = pkgs.callPackage ./cottage.nix { };
...
#define HOWM_PATH "/run/current-system/sw/bin/howm"
...

{
  ...
  options = {
    services.xserver.windowManager.howm = {
      enable = mkEnableOption "howm";
      extraSessionCommands = mkOption {
        default = "";
        type = types.lines;
        description = ''
          Shell commands executed just before howm is started.
        '';
      };
      package = mkPackageOption pkgs "howm" {
        example = '' '';
      };
    };
  };

  config = lib.mkIf (builtins.elem "howm" config.my.window-managers) {
