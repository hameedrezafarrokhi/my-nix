...
/* Passphrase caching: kept ONLY in the kernel session keyring
 * (add_key()/keyctl, see passphrase_cache.c) -- never written to
 * disk, never present in any config file, automatically gone at
 * logout/reboot (whenever the session keyring is destroyed). Global
 * default plus per-device override, same matching rules as
 * DEVICE_OVERRIDES. "Forget Passphrase" in the device menu clears a
 * specific entry early if you don't want to wait for logout. */
 ...
 * MTP (Android / phones)
 * ==================================================================
 * No gvfs, no glib -- attachment is detected via a udev monitor
 * (added to the same poll() loop as everything else, so still 0%
 * idle CPU) watching for USB devices carrying the ID_MTP_DEVICE=1
 * property, which is set by the mtp-probe udev rule that ships with
 * libmtp (installed automatically as jmtpfs's own dependency, so if
 * jmtpfs is installed, detection works with nothing extra to enable).
 * Mounting shells out to `jmtpfs`/`fusermount` -- a real FUSE mount,
 * so ordinary programs (cp, rsync, a terminal, any file manager) can
 * read and write it like any other directory, not just GIO-aware
 * apps. Requires the `jmtpfs` package. */
#define ENABLE_MTP 1

#define MTP_MOUNT_DIR_TEMPLATE "~/Phones/{NAME}"  /* {NAME} -> sanitized vendor+model, {SERIAL} also available */
