...
//history patch
void archive_history_file(const char *history_path, const char *home_dir)
{
    ...
    snprintf(archive_path, sizeof(archive_path), "%s/history_%s.json", home_dir, timestamp);
    rename(history_path, archive_path);
}

int is_duplicate_notification(const char *history_path, char **lines, int num_of_lines)
{
    ...
    char buffer[8192];
    char last_msg[4096] = {0};
    json_object *last_obj = NULL;

    fseek(history_file, 0, SEEK_END);
    long file_size = ftell(history_file);
    if (file_size > 0) {
        long read_start = file_size > 8000 ? file_size - 8000 : 0;
        fseek(history_file, read_start, SEEK_SET);

        char *json_buf = malloc(file_size - read_start + 1);
        if (json_buf) {
            fread(json_buf, 1, file_size - read_start, history_file);
            json_buf[file_size - read_start] = '\0';

            char *last_brace = strrchr(json_buf, '}');
            if (last_brace) {
                char *first_brace = last_brace;
                while (first_brace > json_buf && *first_brace != '{') first_brace--;
                if (*first_brace == '{') {
                    int len = last_brace - first_brace + 1;
                    char *obj_str = malloc(len + 1);
                    if (obj_str) {
                        strncpy(obj_str, first_brace, len);
                        obj_str[len] = '\0';
                        last_obj = json_tokener_parse(obj_str);
                        free(obj_str);
                    }
                }
            }
    ...
    if (last_obj) {
        json_object *msg_obj = json_object_object_get(last_obj, "message");
        if (msg_obj) {
            strncpy(last_msg, json_object_get_string(msg_obj), sizeof(last_msg) - 1);
        }
        json_object_put(last_obj);
    }

    fclose(history_file);
    return (strcmp(current_msg, last_msg) == 0);
}
...
void save_to_history(char **lines, int num_of_lines)
{
    ...
    struct tm *tm_info;
    FILE *history_file;
    json_object *entry_obj, *existing_array;
    int line_count = 0;
    ...
    if (is_duplicate_notification(history_path, lines, num_of_lines)) {
        return;
    }
    ...
    for (int i = 0; i < num_of_lines; i++) {
        ...
        if (i < num_of_lines - 1) strcat(message, " ");
    }
    ...
    json_object_object_add(entry_obj, "timestamp", json_object_new_string(timestamp));
    json_object_object_add(entry_obj, "message", json_object_new_string(message));

    existing_array = json_object_from_file(history_path);
    if (!existing_array) {
        existing_array = json_object_new_array();
    }

    line_count = json_object_array_length(existing_array);

    if (line_count >= MAX_HISTORY_LINES) {
        json_object_put(existing_array);
        archive_history_file(history_path, home_dir);
        existing_array = json_object_new_array();
        line_count = 0;
    }

    json_object_array_add(existing_array, entry_obj);
