...
int mtp_perform_mount(MtpDevice *d, char **out_error) {
    ...
    char *argv[4] = { (char *)"gio", (char *)"mount", uri, NULL };
    int code = 0;
    ...
    if (code != 0) {
        if (out_error) {
            char b[400];
            snprintf(b, sizeof(b),
                     "gio mount exited with status %d (phone unlocked and set to file-transfer/MTP "
                     "mode? is the gvfs package installed with its D-Bus service active?)", code);
            *out_error = strdup(b);
        }
        return 0;
    }
...
int mtp_perform_unmount(MtpDevice *d, char **out_error) {
    ...
    char *argv[5] = { (char *)"gio", (char *)"mount", (char *)"-u", uri, NULL };
    int code = 0;
    spawn_and_wait(argv, &code);

    if (code != 0) {
        if (out_error) *out_error = strdup("gio mount -u failed (still busy? close any open files/apps using it)");
        return 0;
    }
