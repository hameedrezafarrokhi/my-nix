...
main (int argc, char *argv[]) {
    ...
    void dimpause(int signo)
    {
        (void)signo;
        dimpaused = !dimpaused;
    }
    // void lockpause() { lockpaused = !lockpaused; }
    void lockpause(int signo)
    {
        (void)signo;
    ...
    for (;;) {
        ...
        // lock screen
        if (lock_conf && !*locked && lidle == lock_idle) { lock(0); }
...
stdenv.mkDerivation rec {
 ...
 #makeFlags = [ "PREFIX=$(out)" ];
  ...
  buildPhase = ''
    $CC -w gloom.c -lX11 -lXext -lXrandr -lXfixes -lXi -o gloom
  '';

  installPhase = ''
    install -Dm755 gloom $out/bin/gloom
  '';
