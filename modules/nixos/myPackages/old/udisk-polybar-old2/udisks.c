    ...
    *out_objects = NULL;
    ...
    while (dbus_message_iter_get_arg_type(&objects) == DBUS_TYPE_DICT_ENTRY) {
        ...
        if (block.path) {
            ...
            free(block.loop_backing_file); free(block.crypto_backing_device);
...
int udisks_loop_mount_file(const char *file_path, char **out_block_path,
    ...
    *out_mount_point = NULL;
    ...
    /* Mounting a freshly-created loop device can race UDisks' own
     * property-refresh briefly; a short blocking retry loop here is
     * simpler and more reliable than wiring up a signal wait for a
     * one-shot action, and it only ever runs while the user is
     * actively waiting on this one menu click. */
    char *mount_err = NULL;
    for (int attempt = 0; attempt < 5; attempt++) {
        if (udisks_mount(loop_path, out_mount_point, &mount_err)) {
            free(mount_err);
    ...
    if (out_error) *out_error = strdup("mounted the ISO as a loop device, but could not mount its filesystem");
    *out_block_path = loop_path;
    return 0;
}
...
int udisks_loop_delete(const char *loop_block_object_path, char **out_error) {
    ...
    udisks_unmount(loop_block_object_path, 0, &unmount_err);
    free(unmount_err); /* fine if it was never mounted */
