...
`jmtpfs` for phone mounting, and a notification daemon (dunst, mako,
etc) if you want `NOTIFY_ON_*` to do anything. Neither is required to
build or to use the disk-management features.
...
the kdeconnect module. Every object has a `"kind"` field, `"disk"` or
`"phone"`, since the array holds both.

Disk fields: `id`, `name`, `node`, `label`, `uuid`, `fs_type`,
...
**Caching** lives *only* in the kernel's session keyring
(`add_key`/`request_key`/`keyctl_read_alloc`/`keyctl_revoke` --
`passphrase_cache.c`) -- never written to disk, never in a config
file, gone automatically at logout/reboot. `REMEMBER_PASSPHRASE_DEFAULT`
is on by default, with per-device overrides in
`PASSPHRASE_CACHE_OVERRIDES`; "Forget Cached Passphrase" clears one
early without waiting for logout. If a cached passphrase gets rejected
(e.g. you changed it outside this tool), it's dropped automatically
and you're prompted fresh.
...
No gvfs, no glib, no libmtp linked into this binary. Two pieces:
...
- **Detection**: a `libudev` monitor (folded into the daemon's
  ...
  `libmtp` -- which `jmtpfs` itself depends on, so if `jmtpfs` is
  installed, detection works with nothing extra to configure.
- **Mounting**: shells out to `jmtpfs <dir> -device=bus,dev`, a real
  FUSE mount -- so `cp`, `rsync`, a terminal, or any file manager can
  read/write it like any other directory, not just GIO-aware apps
  (which is the trap with a bare `gio mount mtp://...`). Unmounting
  shells out to `fusermount -u`. Requires the `jmtpfs` package.
...
(`CUSTOM_MTP_MENU_ENTRIES`). Mount directory is
`MTP_MOUNT_DIR_TEMPLATE` (default `~/Phones/{NAME}`, `{SERIAL}` also
available for disambiguating identical models).
...
- Multiple phones attached at once are disambiguated by USB bus/device
  number (passed to `jmtpfs -device=`), so two phones won't collide.
