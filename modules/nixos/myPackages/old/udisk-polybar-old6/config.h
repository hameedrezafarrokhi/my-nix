...
/* How long to wait for the mount to actually appear (checked via
 * /proc/mounts, not just "is the process still alive" -- some tools
 * daemonize immediately on success, which would otherwise look
 * indistinguishable from having already exited) before giving up and
 * reporting failure. */
#define MTP_MOUNT_TIMEOUT_SEC 8
...
#define CONFIRM_MTP_MOUNT   1
#define CONFIRM_MTP_UNMOUNT 1
