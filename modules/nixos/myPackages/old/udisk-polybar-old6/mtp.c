...
int mtp_perform_mount(MtpDevice *d, char **out_error) {
    ...
    if (is_path_mounted(mountdir)) {
        /* already mounted (e.g. this MtpDevice came from a fresh
         * process and a previous attempt actually succeeded) */
        free(d->mount_point);
    ...
    for (int elapsed_ms = 0; elapsed_ms < MTP_MOUNT_TIMEOUT_SEC * 1000; elapsed_ms += 150) {
        char buf[512];
        ssize_t rn;
    ...
    /* failed -- make sure nothing's left half-running */
    if (child_alive) { kill(pid, SIGTERM); usleep(200000); kill(pid, SIGKILL); }
    ...
    if (out_error) {
        ...
        if (child_exit_code == 127) {
            ...
            snprintf(b, sizeof(b),
                     "%s never mounted anything (timed out after %ds). Phone unlocked and set to "
                     "file-transfer/MTP mode?", MTP_MOUNT_CMD, MTP_MOUNT_TIMEOUT_SEC);
        }
        *out_error = strdup(b);
    }
    return 0;
...
int mtp_perform_unmount(MtpDevice *d, char **out_error) {
    ...
    if (!is_path_mounted(mountdir)) {
        if (d->mount_pid > 0 && pid_alive(d->mount_pid)) kill(d->mount_pid, SIGKILL);
        state_remove_mtp_mount(key);
        rmdir(mountdir);
        free(d->mount_point);
        d->mount_point = NULL;
...
int mtp_scrcpy_stop(MtpDevice *d, char **out_error) {
    ...
    kill(d->scrcpy_pid, SIGTERM);
    usleep(200000);
    if (pid_alive(d->scrcpy_pid)) kill(d->scrcpy_pid, SIGKILL);
