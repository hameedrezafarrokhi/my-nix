...
- **Mounting**: `MTP_MOUNT_CMD` (default `"go-mtpfs"` -- the only MTP
  ...
  to change.
- **Success detection** watches `/proc/mounts` for the mount point to
  actually appear (up to `MTP_MOUNT_TIMEOUT_SEC`, default 8s) rather
  than trusting "is the process still alive" -- some FUSE tools
  daemonize to the background immediately on success, which would
  otherwise look identical to having already crashed.
- **Unmounting** always ends with `fusermount -u <mountpoint>` --
  ...
  default), it gets a `SIGTERM` first, on the theory that letting a
  well-behaved tool clean up after itself is nicer than yanking the
...
- **Unplugging without unmounting first** is handled the same way --
  ...
  recorded as mounted, and runs the exact same SIGTERM-then-
  `fusermount -u` cleanup automatically. Also swept for on daemon
