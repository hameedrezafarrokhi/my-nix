...
/* mtp-mounts file format: one "device_key\tpid\tmount_point\n" triple
 * per line -- same small-flat-file reasoning as the other two state
 * files above. */

void state_set_mtp_mount(const char *device_key, pid_t pid, const char *mount_point) {
    if (!device_key || !device_key[0]) return;
    state_remove_mtp_mount(device_key); /* drop any stale entry for this key first */

    char *path = state_path("mtp-mounts");
    if (!path) return;
    FILE *f = fopen(path, "a");
    if (f) {
        fprintf(f, "%s\t%ld\t%s\n", device_key, (long)pid, mount_point ? mount_point : "");
        fclose(f);
    }
    free(path);
}
...
void state_remove_mtp_mount(const char *device_key) {
    ...
    if (in) {
        ...
        while (fgets(line, sizeof(line), in)) {
            ...
            if (keylen == strlen(device_key) && !strncmp(line, device_key, keylen)) continue; /* dropped */
            kept = realloc(kept, sizeof(char *) * (n + 1));
            kept[n++] = strdup(line);
        }
        fclose(in);
...
int state_get_mtp_mount(const char *device_key, pid_t *out_pid, char **out_mount_point) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *tab1 = '\0';
        ...
        if (!tab2) continue;
        *tab2 = '\0';
        if (out_pid) *out_pid = (pid_t)atol(tab1 + 1);
        if (out_mount_point) *out_mount_point = strdup(tab2 + 1);
        found = 1;
        break;
    }
    fclose(f);
