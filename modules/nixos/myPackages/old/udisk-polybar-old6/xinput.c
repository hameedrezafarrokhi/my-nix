...
char *xinput_show(Display *dpy, int x, int y, const char *prompt, const char *initial, int mask) {
    ...
    while (!confirmed && !cancelled) {
        ...
        if (ev.type == Expose || ev.type == FocusIn) {
            ...
            char shown[BUF_CAP * 3 + 8];
            if (mask) {
                size_t o = 0;
                for (int i = 0; i < len && o + 3 < sizeof(shown); i++) {
                    shown[o++] = '\xE2'; shown[o++] = '\x97'; shown[o++] = '\x8F'; /* U+25CF bullet */
                }
                shown[o] = '\0';
                strncat(shown, "\xE2\x96\x8F", sizeof(shown) - strlen(shown) - 1); /* trailing thin cursor block */
            } else {
                snprintf(shown, sizeof(shown), "%s\xE2\x96\x8F", buf);
            }
