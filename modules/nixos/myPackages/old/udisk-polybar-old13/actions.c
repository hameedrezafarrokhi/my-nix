...
static void mtp_substitute_raw(const char *tmpl, const MtpDevice *m, char *out, size_t outlen) {
    ...
    for (const char *p = tmpl; *p && o < outlen - 1; ) {
        ...
        else if (!strncmp(p, "{MOUNTPOINT}", 12)) { rep = mountdir; skip = 12; }
...
/* Runs a config.h shell-command string (MTP_MOUNT_SHELL_CMD /
 * MTP_UNMOUNT_SHELL_CMD) via `sh -c` -- fire-and-forget, exactly like
 * any other menu-entry command, no MTP-specific behavior wrapped
 * around it: no success/failure detection, no notification, no path
 * substitution. This module has no opinion about mounting; it just
 * runs what you configured. */
...
#define MBUF() pool[pn++]
    ...
    if (m->usage_valid) {
        ...
        char *b1 = MBUF(); snprintf(b1, 600, "Usage: unavailable");
        add_info(items, &n, b1);
    }
    if (m->scrcpy_pid > 0) {
    ...
    /* Everything below is always shown -- this module has no concept
     * of "mounted" to gate any of it on anymore. Mount and Unmount run
     * whatever you configured, no differently than a custom entry. */
    if (MENU_ACTION_MTP_MOUNT) add_action(items, &n, "Mount", ACT_MTP_MOUNT, 1);
    if (MENU_ACTION_MTP_UNMOUNT) add_action(items, &n, "Unmount", ACT_MTP_UNMOUNT, 1);
    if (MENU_ACTION_MTP_OPEN_FILE_MANAGER)
        add_action(items, &n, "Open in File Manager", ACT_MTP_OPEN_FILE_MANAGER, 1);
    if (MENU_ACTION_MTP_COPY_MOUNT_PATH)
        add_action(items, &n, "Copy Mount Path", ACT_MTP_COPY_MOUNT_PATH, 1);
    if (MENU_ACTION_MTP_DOWNLOAD)
        add_action(items, &n, "Download from Phone...", ACT_MTP_DOWNLOAD, 1);
    if (MENU_ACTION_MTP_UPLOAD)
    ...
    switch (id) {
        case ACT_MTP_MOUNT: {
            ...
            if (CONFIRM_MTP_MOUNT) {
                char q[300]; snprintf(q, sizeof(q), "Run Mount for %s?", m->display_name);
                go = confirm_dialog(dpy, x, y, q);
            }
            if (go) run_shell_command(MTP_MOUNT_SHELL_CMD);
            break;
        }
        case ACT_MTP_UNMOUNT: {
            int go = 1;
            if (CONFIRM_MTP_UNMOUNT) {
                char q[300]; snprintf(q, sizeof(q), "Run Unmount for %s?", m->display_name);
                go = confirm_dialog(dpy, x, y, q);
            }
            if (go) run_shell_command(MTP_UNMOUNT_SHELL_CMD);
            break;
        }
        ...
        case ACT_MTP_COPY_MOUNT_PATH: {
            char mountdir[1300];
            mtp_fixed_mount_dir(mountdir, sizeof(mountdir));
            copy_to_clipboard(dpy, mountdir);
            break;
        }
        case ACT_MTP_DOWNLOAD: {
            ...
            mtp_fixed_mount_dir(mountdir, sizeof(mountdir));
            int count = 0;
            char **paths = filepicker_choose_at(dpy, x, y, mountdir, 1, &count);
            if (count > 0) {
        ...
        case ACT_MTP_UPLOAD: {
            ...
            if (count > 0) {
                char mountdir[1300];
                mtp_fixed_mount_dir(mountdir, sizeof(mountdir));
                mtp_transfer_copy_files(dpy, x, y, paths, count, mountdir);
            }
            for (int i = 0; i < count; i++) free(paths[i]);
            free(paths);
            break;
