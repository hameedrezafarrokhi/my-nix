 ...
 * downtime. */
/* Runs the user's own MTP_UNMOUNT_SHELL_CMD (config.h) -- exactly the
 * same command the Unmount menu row runs, nothing added -- when a
 * phone that was previously attached disappears from a fresh udev
 * enumeration. This is the one thing this daemon still does
 * proactively about unmounting: run *your* command for you on
 * unplug, since you can't click a menu row for a phone that's already
 * gone. Fire-and-forget, no success/failure handling, matching how
 * Unmount itself behaves everywhere else now. */
static void run_unmount_cmd_for_departed(const MtpDeviceList *old_list, const MtpDeviceList *fresh) {
    for (int i = 0; i < old_list->count; i++) {
        char k[64];
        mtp_device_key(&old_list->items[i], k, sizeof(k));
        int still_present = 0;
        for (int j = 0; j < fresh->count; j++) {
            char k2[64];
            mtp_device_key(&fresh->items[j], k2, sizeof(k2));
            if (!strcmp(k, k2)) { still_present = 1; break; }
        }
        ...
        if (pid == 0) {
            ...
            execlp("/bin/sh", "sh", "-c", MTP_UNMOUNT_SHELL_CMD, (char *)NULL);
            _exit(127);
        }
        /* fire-and-forget -- not our concern whether it worked, same as every other menu-entry command */
    }
}

/* Same idea, for a tracked scrcpy process whose phone is no longer
 * present -- this is the fix for scrcpy getting left running
 * (sometimes unkillably, if it had spawned its own helper processes
 * we weren't targeting -- see kill_group() in mtp.c) when a phone is
 * unplugged without stopping it first. */
...
int daemon_run(int json_mode) {
    ...
    if (have_mtp) {
        ...
        scrcpy_cleanup_orphans(&mtp_list); /* recover any scrcpy left running from before a daemon restart */
    ...
    for (;;) {
        ...
        if (fds[1].revents & POLLIN) {
            ...
            if (b == PIPE_BYTE_RELOAD) {
                ...
                if (have_mtp) mtp_list_refresh_usage(&mtp_list); /* opportunistic usage stats only -- no mount state left to resync, see mtp.h */
                print_state(&list, have_mtp ? &mtp_list : NULL, &last, json_mode);
        ...
        if (have_mtp && (fds[2].revents & POLLIN)) {
            ...
            run_unmount_cmd_for_departed(&mtp_list, &fresh);
