...
static void mtp_substitute(const char *tmpl, const MtpDevice *m, char *out, size_t outlen) {
    ...
    { char mountdir[1300]; mtp_fixed_mount_dir(mountdir, sizeof(mountdir)); shell_quote(mountdir, mp_q, sizeof(mp_q)); }
...
static void render_one_mtp(const MtpDevice *m, char *out, size_t outlen) {
    ...
    /* No mounted/unmounted concept from our side anymore -- one icon
     * for "attached", full stop. */
    const char *icon = ICON_PHONE_GENERIC;
    strncat(body, icon, sizeof(body) - strlen(body) - 1);
...
void render_json(const DeviceList *list, const MtpDeviceList *mtp_list) {
    ...
    if (mtp_list) {
        for (int i = 0; i < mtp_list->count; i++) {
            ...
            { char mountdir[1300]; mtp_fixed_mount_dir(mountdir, sizeof(mountdir)); json_escape(mountdir, mp_e, sizeof(mp_e)); }

            printf("%s{"
                   "\"kind\":\"phone\",\"id\":\"%s\",\"name\":\"%s\",\"serial\":\"%s\","
                   "\"vendor\":\"%s\",\"model\":\"%s\",\"mount_point\":\"%s\","
