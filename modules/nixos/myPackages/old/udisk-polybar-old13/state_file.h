...
/* Tracking a spawned scrcpy process per device (so "Run/Stop scrcpy"
 * in the Android menu works correctly from a fresh one-shot process,
 * and so a still-running scrcpy can be found and offered a Stop even
 * after the phone was unplugged and replugged). 0 as a pid means "not
 * tracked/not running". */
...
/* Every currently-recorded device_key with a tracked scrcpy process,
 * for the daemon's unplug sweep. Malloc'd array of malloc'd strings;
