...
int mtp_perform_mount(MtpDevice *d, char **out_error) {
    ...
    /* Unconditionally clear anything registered on this exact path
     * before trying again -- no judgment call about whether it's
     * "really" stale, just always start from a known-clean slate.
     * Harmless (fast no-op) if nothing's there. */
