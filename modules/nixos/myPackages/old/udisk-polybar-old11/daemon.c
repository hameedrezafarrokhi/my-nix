    ...
    for (;;) {
        ...
        if (fds[1].revents & POLLIN) {
            ...
            if (b == PIPE_BYTE_RELOAD) {
                ...
                if (have_mtp) {
                    /* Full rebuild, not just a usage refresh -- unlike
                     * disks (which get this for free via UDisks' own
                     * D-Bus signals, regardless of which process did
                     * the mounting), nothing tells this daemon an MTP
                     * mount/unmount happened elsewhere except this
                     * explicit nudge, so it has to actually re-check
                     * is_mounted itself, not just refresh bytes-free
                     * on devices it already *thought* were mounted. */
