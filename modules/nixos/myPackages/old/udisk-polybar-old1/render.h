 ...
 * joined by SEPARATOR, honoring every BAR_SHOW_* toggle in config.h)
 * to stdout, newline-terminated. */
void render_bar(const DeviceList *list);

/* Prints a JSON array (one object per device) to stdout. Schema is
