...
#define MBUF() pool[pn++]
    ...
    switch (id) {
        case ACT_MTP_MOUNT: {
            ...
            /* Unlike disks, nothing else tells the persistent daemon
             * an MTP mount happened -- UDisks' own D-Bus signals give
             * disks this for free regardless of which process did the
             * mounting; there's no equivalent for an external mount
             * command, so it has to be nudged explicitly or the bar/
             * menu just keeps showing stale state forever. */
            action_signal_daemon_reload();
            free(err);
            break;
        }
