 ...
 * leader via setpgid(0,0) before exec, specifically so this catches any
...
/* Full transparency for debugging: exactly what was run and what
 * happened, written to a plain log file every single mount attempt
 * (overwritten each time -- this is "what just happened", not a
 * history). `cat ~/.cache/polybar-udisks/mtp-mount.log` after any
 * Mount click shows precisely the argv this module executed and the
 * command's own combined stdout+stderr, with no guessing needed. */
static void write_mount_log_command(char *const argv[], int argc) {
    const char *cache_home = getenv("XDG_CACHE_HOME");
    char path[1024];
    if (cache_home && cache_home[0]) snprintf(path, sizeof(path), "%s/polybar-udisks", cache_home);
    else { const char *home = getenv("HOME"); snprintf(path, sizeof(path), "%s/.cache/polybar-udisks", home ? home : ""); }
    mkdir_recursive(path);

    char logfile[1100];
    snprintf(logfile, sizeof(logfile), "%s/mtp-mount.log", path);
    ...
    /* Environment this process actually ran with, for comparing
     * against a manual terminal run -- if go-mtpfs behaves
     * differently spawned from here than typed by hand with an
     * identical command line, something in this list is almost
     * certainly why (a WM/session-autostarted polybar commonly gets a
     * much thinner environment and an earlier/different login session
     * than a terminal you open afterward -- which matters here
     * specifically because udev's per-session USB device permissions
     * are granted per logind session, not just per user). */
    fprintf(f, "---- environment ----\n");
    static const char *interesting_vars[] = {
        "PATH", "HOME", "USER", "LOGNAME", "XDG_RUNTIME_DIR", "XDG_SESSION_ID",
        "XDG_SESSION_TYPE", "DISPLAY", "XAUTHORITY", "DBUS_SESSION_BUS_ADDRESS", NULL
    };
    for (int i = 0; interesting_vars[i]; i++) {
        const char *v = getenv(interesting_vars[i]);
        fprintf(f, "%s=%s\n", interesting_vars[i], v ? v : "(unset)");
    }
    fprintf(f, "uid=%d gid=%d\n", (int)getuid(), (int)getgid());
    {
        gid_t groups[64];
        int n = getgroups(64, groups);
        fprintf(f, "supplementary groups:");
        if (n < 0) fprintf(f, " (getgroups failed)");
        else for (int i = 0; i < n; i++) fprintf(f, " %d", (int)groups[i]);
        fprintf(f, "\n");
    }
...
static void write_mount_log_result(int mounted, int child_alive, int child_exit_code, const char *output) {
    const char *cache_home = getenv("XDG_CACHE_HOME");
    char path[1024];
    if (cache_home && cache_home[0]) snprintf(path, sizeof(path), "%s/polybar-udisks", cache_home);
    else { const char *home = getenv("HOME"); snprintf(path, sizeof(path), "%s/.cache/polybar-udisks", home ? home : ""); }
    ...
    if (!f) return;
    fprintf(f, "%s\n---- result ----\nmounted=%d child_alive=%d child_exit_code=%d\n",
            output && output[0] ? output : "(no output captured)", mounted, child_alive, child_exit_code);
    fclose(f);
}
...
int mtp_perform_mount(MtpDevice *d, char **out_error) {
    ...
    /* MTP_MOUNT_PARENT_DIR is the mount point, full stop -- no
     * per-device subdirectory appended. */
    char mountdir[1300];
    expand_home(MTP_MOUNT_PARENT_DIR, mountdir, sizeof(mountdir));
    mkdir_recursive(mountdir);

    if (is_path_mounted(mountdir)) {
        /* Already mounted -- leave it completely alone and report
         * success. This used to unconditionally force-unmount
         * whatever was here first "to start clean", which was actively
         * destructive: clicking Mount again on an already-working
         * mount (which is exactly what happens if there's no visible
         * feedback that the first click already succeeded) tore the
         * working mount down and immediately tried to rebuild it,
         * over and over. A mount that's already there needs zero
        ...
        char key[64];
        mtp_device_key(d, key, sizeof(key));
        state_set_mtp_mount(key, d->mount_pid, mountdir); /* keep the state file's own record in sync (harmless if it already matches) */
    ...
    write_mount_log_command(argv, n);
    ...
    if (pid == 0) {
        ...
        if (devnull >= 0) { dup2(devnull, 0); if (devnull > 2) close(devnull); }
        if (pipefd[1] > 2) close(pipefd[1]);
        /* setpgid(0,0), not setsid() -- we only need this to be its
         * own process group (so kill_group() can signal it and any
         * helpers it spawns together), not a fully detached session.
         * setsid() additionally drops the controlling terminal and
         * starts a brand new session, which is a bigger behavioral
         * difference from "you ran this by hand in your terminal"
         * than we actually need, and worth ruling out as a cause if
         * mounting still behaves differently spawned from here than
         * typed directly. */
        setpgid(0, 0);
        execvp(argv[0], argv);
        _exit(127);
    ...
    /* Success = registered in /proc/mounts. That's it -- no attempt
     * to second-guess whether the mount tool has *fully* finished its
     * own handshake with the phone beyond that (an earlier version of
     * this function tried to verify that with a directory read, which
     * turned out to be actively misleading: an MTP mount can answer a
     * root-directory listing instantly and successfully while still
     * empty, before the phone has actually granted the file-transfer
     * permission -- so that check was passing "successfully" on
     * mounts that were, from the outside, indistinguishable from
     * broken. Plain /proc/mounts registration, checked repeatedly
     * against a real wall clock, is what a simple shell script doing
     * this would check, and is what's actually reliable here). */
    while (now_ms() - start_ms < timeout_ms) {
        ...
        if (is_path_mounted(mountdir)) { mounted = 1; break; }
        ...
        if (!child_alive) {
            ...
            usleep(300000);
            if (is_path_mounted(mountdir)) mounted = 1;
            break;
        }
    ...
    /* Drain anything left in the pipe after the loop exits (e.g. the
     * process printed more right as we stopped watching). */
    { char buf[512]; ssize_t rn;
      while ((rn = read(pipefd[0], buf, sizeof(buf))) > 0) {
          size_t room = sizeof(output) - 1 - out_len;
          size_t take = (size_t)rn < room ? (size_t)rn : room;
          if (take > 0) { memcpy(output + out_len, buf, take); out_len += take; output[out_len] = '\0'; }
    ...
    write_mount_log_result(mounted, child_alive, child_exit_code, output);
    ...
    if (out_error) {
        ...
        if (child_exit_code == 127) {
            ...
            snprintf(b, sizeof(b),
                     "%s is still running but nothing ever showed up in /proc/mounts (timed out after "
                     "%ds). Full command + output logged to ~/.cache/polybar-udisks/mtp-mount.log.",
...
int mtp_scrcpy_start(MtpDevice *d, char **out_error) {
    ...
    pid_t pid = fork();
    if (pid == 0) {
        setpgid(0, 0); /* process group only, not a full setsid() -- same reasoning as the mount command's spawn above */
