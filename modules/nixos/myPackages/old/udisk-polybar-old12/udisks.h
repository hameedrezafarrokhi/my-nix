...
typedef struct {
    ...
    char *partition_table_path; /* set only if has_partition -- the object path of the block device containing this partition's table (e.g. our loop device, for a loop-mounted hybrid ISO) */
 ...
 * mounts it read-only. Handles the common "plain ISO9660" case (the
 * loop device itself gets a Filesystem interface) as well as hybrid/
 * bootable ISOs (isohybrid images with an MBR/GPT partition table,
 * common for Linux distro install media) where the actual mountable
 * filesystem lives on a *partition* of the loop device instead --
 * probed for automatically, no user-visible difference either way.
 * On success *out_block_path is always the LOOP device's own object
 * path (needed later by udisks_loop_delete, regardless of whether a
 * partition of it is what actually got mounted), and
 * *out_mount_point its mount path (malloc'd). */
 ...
 * for "Detach ISO". Also unmounts any partition of the loop device
 * that ended up mounted instead of the loop device itself (see
 * udisks_loop_mount_file). */
