...
#define BAR_SHOW_NAME            1
#define BAR_NAME_MAX_CHARS       14   /* UTF-8-char-safe; 0 = no limit */
...
#define MENU_SHOW_CONNECTION_BUS     0
...
#define FILE_MANAGER_CMD "xdg-open {MOUNTPOINT}"
 ...
 * fine: "go-mtpfs -allow-other"), then MTP_MOUNT_PARENT_DIR is
 * appended as the final argument -- i.e. this runs exactly
 * `go-mtpfs ~/Phone`, nothing else added to the command line.
 * Whatever you set here is expected to behave like an ordinary FUSE
 * mount tool given a mountpoint as its last argument: either it stays
 ...
 * (see MTP_UNMOUNT_FUSERMOUNT_CMD below), so switching tools never
 * needs a code change, just this line. */
#define MTP_MOUNT_CMD "go-mtpfs"

/* How long to wait for the mount to appear in /proc/mounts before
 * giving up and reporting failure. Generous by default: the most
 * common reason this takes a while is the phone's own file-transfer
 * permission prompt sitting unanswered on its screen, not anything
 * actually being slow. */
#define MTP_MOUNT_TIMEOUT_SEC 25

/* The mount point -- used exactly as given, no subdirectory or device
 * name appended. Only one phone can be mounted here at a time; if you
 * ever need more than one phone mounted simultaneously, point this at
 * a path that already includes something device-specific yourself
 * (there's no way to do that generically here without reintroducing
 * the auto-appended-subdirectory behavior this deliberately doesn't
 * have anymore). */
...
#define MTP_AUTO_SPAWN_SCRCPY 0   /* launch scrcpy automatically whenever a phone is detected (not just mounted -- scrcpy doesn't need an MTP mount at all) */
...
#define NOTIFY_ON_MTP_ATTACHED        0  /* phone detected but not yet mounted -- no automount for MTP: MTP is too slow/quirky for a surprise background mount, always a manual click */
...
#define CONFIRM_ISO_DETACH                0   /* non-destructive (it's read-only media you can just remount), off by default unlike CONFIRM_POWER_OFF */
