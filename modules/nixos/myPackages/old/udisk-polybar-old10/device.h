...
typedef struct {
    ...
    int is_encrypted;           /* this row is (or unlocks to) a LUKS container */
    int is_locked;               /* the container itself, not yet unlocked -- no filesystem, no mount actions, just "Unlock" */
    char *parent_luks_path;      /* set on the cleartext row once unlocked -- the container's object path, used for "Lock" */
    char *parent_luks_uuid;      /* the container's own UUID (different from this row's uuid, which is the inner filesystem's) -- used as the passphrase cache key */
...
/* Effective passphrase-caching decision for `d` (PASSPHRASE_CACHE_OVERRIDES,
 * falling back to REMEMBER_PASSPHRASE_DEFAULT). Matched the same way
 * as device_should_automount -- against `d`'s own fields, so pass the
 * *container* row (its uuid is the container's own, whether locked or
 * about to be looked up via parent_luks_uuid on a cleartext row). */
